...
const Add = ({token}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3 ">
      ...
      <button type="submit" disabled={loading} className={`w-28 py-3 mt-4 ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-black cursor-pointer'} text-white`}>{loading ? 'Adding...' : 'ADD'}</button>
    </form>
  )
}
